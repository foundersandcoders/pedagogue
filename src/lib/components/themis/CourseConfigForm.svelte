<script lang="ts">
  ...
  $: isDataReady =
    formData?.logistics?.totalWeeks !== undefined &&
  ...
  function validateForm(): boolean {
    ...
    if (
      formData.learners?.teamBased &&
      (!formData.learners?.teamSize || formData.learners.teamSize < 2)
    ) {
      if (!errors.learners) errors.learners = {};
...
<style>
  ...
  .form-field.indent {
    margin-left: 2rem;
    padding-left: 1rem;
    border-left: 3px solid var(--palette-line);
  }
