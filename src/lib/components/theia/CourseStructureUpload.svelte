<script lang="ts">
	...
	async function handleFile(file: File) {
		...
		try {
			...
			error = err instanceof Error ? getUserFriendlyError(err) : 'Invalid course JSON';
			uploadState = 'error';
			dispatch('uploadError', { error });
		}
	}
		...
		{:else if uploadState === 'success' && uploadedCourse}
			<div class="upload-status success">
				...
				<div class="course-stats">
					...
					<span>•</span>
					<span
						>{uploadedCourse.arcs.reduce((sum, arc) => sum + arc.modules.length, 0)} modules</span
					>
					<span>•</span>
				...
				{#if validationWarnings.length > 0}
					<div class="warnings">
						<p class="warnings-header">⚠️ Warnings ({validationWarnings.length}):</p>
						<ul>
				...
				<div class="actions">
					...
					<button on:click|stopPropagation={reset}> Upload Different File </button>
				</div>
			</div>
