<script lang="ts">
	...
	import type { ErrorSeverity } from '$lib/errors/generation-errors.js';
...
<div class="error-alert {severityClass}" role="alert">
	...
	<div class="error-alert-actions">
		{#if showRetry && error.retryable && onRetry}
			<button class="error-alert-button error-alert-retry" on:click={handleRetry}>
				Retry
			</button>
		{/if}

		{#if showDetails}
			<button class="error-alert-button error-alert-details" on:click={toggleDetails}>
				{detailsExpanded ? 'Hide' : 'Show'} Details
			</button>
		{/if}

		{#if showDismiss && onDismiss}
			<button class="error-alert-button error-alert-dismiss" on:click={handleDismiss}>
				Dismiss
			</button>
		{/if}
	</div>
